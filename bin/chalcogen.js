...
if (argv.platforms) {
  ...
  console.log("  sauceunit: Run tests on saucelabs.com using their unit testing feature")
...
} else if (argv.platform == 'sauceunit') {
  var sauce = require('../lib/runner-sauceunit');
  sauce.run(wdMocha, {
    environments: nconf.get('chalcogen:environments'),
    username: nconf.get('chalcogen:username') || nconf.get('CI_COMMITTER_USERNAME') || nconf.get('USERNAME') || nconf.get('USER'),
    projectName: nconf.get('name'),
    ...
    process.exit(wdMocha.finalLog2(err, res) ? 0 : 1)
  });
